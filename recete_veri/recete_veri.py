...
class AllReceteSpider(Spider):
    name = "all_recete"
    allowed_domains = ["recete.com"]

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.target_barcode = "5052197052794"
        self.found_target = False

        with open("categories_recete.json", "r", encoding="utf-8") as f:
    ...
    def parse(self, response):
        ...
        for product in products:
            ...
            price = product.css('div.current-price strong.product-price::text').get()
    ...
    def parse_product_detail(self, response):
        ...
        barcode = response.css('div#product-code span::text').get()
        if barcode:
...
process = CrawlerProcess(settings={
    "FEEDS": {
        "recete_all_products.json": {"format": "json", "encoding": "utf-8"},
    },
    "LOG_LEVEL": "ERROR",
